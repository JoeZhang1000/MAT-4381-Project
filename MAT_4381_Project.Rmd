...
cat("Training MSE (adjusted for #parameters):", 
    training_MSE*nrow(train_set_dataset)/(nrow(train_set_dataset) - length(coef(stepwise_model))), "\n")
...
plt.title('Histogram of Simulated Distribution of Prior for $\\sigma^2$ ($IG(9, 125)$)')
...
beta_estimates = pd.DataFrame(index=train_set.columns[0:num_predictors], columns=["Beta Estimate", "95% Credible Region"])
...
for i in range(num_predictors):
    ...
    mean_est = subaxis.axvline(x=mu_post[i], color='c', linestyle='--', linewidth=5, label = 'Posterior Beta Coefficient Estimate for Predictor')
...
fig.suptitle('Histograms of Simulated Marginal Posterior Distributions and Credible Regions for Beta Coefficients of Predictors', fontsize=45, y=0.98)
...
\\
\\
Table 1: Summary of the Bayes estimates for the model parameters. It contains the Bayes estimates under the squared error loss, the variance of the estimates, and the 95\% credible region.
```
We can see that our estimated Bayesian linear regression model is $\hat{Y} = 29.2027 - 0.119609X_1 + 0.0434526X_2 + 0.0626662X_3 - 12.4738 X_4 + 4.16421 X_5 - 0.00535482 X_6 - 1.43013 X_7 + 0.333189 X_8 - 0.0142721 X_9 - 0.807487 X_{10} + 0.0119776 X_{11} - 0.624253 X_{12}$. From the 95% credible regions of the parameters, we can see that for the predictors $X_3$ (INDUS) and $X_6$ (AGE), their credible regions contain 0. This means that there's a 95% probability that 0 is a value for the parameters and hence, the predictors are not statistically significant. The credible regions for the other parameters don't contain 0, which means that they are statistically significant to the model.
...
beta_estimates_comparison = beta_estimates.rename(columns={'Beta Estimate': 'Bayesian Beta Estimate'}).drop('95% Credible Region', axis=1)

beta_estimates_comparison['Beta Estimate Difference (OLS - Bayesian)'] = beta_estimates['Beta Estimate'] - OLS_beta_estimates['betaEstimate']
...
We now compare the variance of the response variable (Median House Prices) under the ordinary least squares and the Bayesian framework. Under the ordinary least squares framework, the estimate for the variance is the mean squared error, or the mean sum of squares residuals. We obtain an estimate of 21.66596 (the training MSE from OLS). Under the Bayesian framework, we get that the estimate for the variance under the squared error loss is 21.506 and the MAP estimate is 21.242. This shows that the estimate for the variance of the median house price is similar under both the ordinary least squares and the Bayesian framework.

# Conclusion/Future Steps

All in all, we explored the differences between Ordinary Least Squares Regression (Frequentist Method) and the Bayesian method of settings priors to find posterior distributions for the model parameters. We analyzed the Boston Housing dataset using both OLS and Bayesian models. Overall, we found that the OLS and Bayesian estimates for the Beta coefficients and $\sigma^2$ were similar in both frameworks. Using the prior we picked for the Bayesian model, the estimates for the parameters are similar for both the ordinary least squares model and the Bayesian linear regression model. Utilizing different parameters for the priors in the Bayesian model could lead to vastly different estimates and affect the signifinance of the predictors. In the future, we could further explore in the Bayesian model prediction of new observations, hypothesis testing of multiple parameters, joint credible regions, choosing various types of distributions as priors (such as Jeffrey's prior), and transformations of our data to fix normality issues. 

# References

Note: These are not cited in proper format yet.

3. <https://www.researchgate.net/publication/333917874_Bayesian_Linear_Regression#pf18>

4. https://online.stat.psu.edu/stat857/node/155/

Introduction to Linear Regression Analysis, sixth edition, 2021. Douglas C.
