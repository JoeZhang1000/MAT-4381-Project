...
stepwise_model <- lm(MEDV ~ CRIM + ZN + NX + RM + DIS + RAD + TAX + PTRATIO + B + LSTAT, data = train_set_dataset)
...
We must now choose the prior parameters for each of the prior distributions. We start with setting the parameters for \$\\sigma\^2 \\sim IG(a,b)\$. As we are not experts in the field of predicting home prices, we do not know the specifics of the variability of home prices in Boston. Therefore, we chose to
...
beta_estimates = pd.DataFrame(index=data.columns[0:num_predictors], columns=["Beta Estimate", "SD Estimate for Beta", "95% Credible Region"])
...
for i in range(num_predictors):
    ...
    subaxis = axes[i]
    n, bins, patches = subaxis.hist(beta_hat[:, i], bins=int(sampling_size / 1000), color=colors(i), alpha=0.75)
    low_cr = subaxis.axvline(x=value_at_25, color='r', linestyle='--', label = 'Lower 95% CR Bound for Estimate')
    high_cr = subaxis.axvline(x=value_at_975, color='darkred', linestyle='--', label = 'Upper 95% CR Bound for Estimate')
    mean_est = subaxis.axvline(x=mu_post[i], color='b', linestyle='--', label = 'Posterior Beta Coefficient Estimate for Predictor')

    subaxis.set_xlabel(f'Beta Value for Predictor {i}')
    subaxis.set_ylabel('Frequency')
    subaxis.set_title(f'{data.columns[i]}', fontsize=12)

lines_labels = [ax.get_legend_handles_labels() for ax in fig.axes][1:2]
lines, labels = [sum(lol, []) for lol in zip(*lines_labels)]
fig.legend(lines, labels, loc='upper center', ncol=3, fontsize=18, bbox_to_anchor=(0.5, 0.96))

# Add an overall title
fig.suptitle('Marginal Posterior Distributions and Confidence Intervals for Predictors', fontsize=25, y=0.98)
...
print(tabulate(beta_estimates, headers='keys', tablefmt='grid',  numalign="left", stralign="left"))
