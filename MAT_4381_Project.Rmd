...
There's another way to test whether predictors are significant to the model, but instead of testing individual parameters only, we can test a subset of all parameters. This is known as the ANOVA test (we will look at one F-test). We won't explore this in too much detail as this is not the primary topic of this paper.
...
Lastly, we can test the null hypothesis that the extra parameters in the full model are not significant versus the alternative that they are significant using the F-test. As seen in the table, we can see that the p-value for the test is around 65%, which is very high, indicating that we do not reject the null hypothesis. This leads to way to us choosing the reduced model as a better choice due to it removing non-significant predictors.
...
For setting the parameters for the joint prior for the $\beta$ coefficients (given $\sigma^2$), we will set $\boldsymbol{m}=0$ (mean of each beta coefficient is set to 0) and $V$ to $10I$. Now, when we set the means to 0 and $V^{-1}$ to $\lambda I$ (in other words $V=\frac{1}{\lambda}I$), where $I$ is the identity matrix and $\lambda \in \mathbb{R}$, we can see that the estimates for the $\beta$ parameters are the same as in the frequentist method of ridge regression. The larger $\lambda$ is, the more shrinking of non-significant estimates occur towards 0. The purpose of this is to attempt to not overfit the training data and be able to more easily generalize to unseen data by not capturing as much noise and potentially unexplanatory predictors. We set $\lambda=2$ to balance the potential for overfitting versus underfitting the model (too large a $\lambda$ value could lead to underfitting).

We start with setting the parameters for \$\\sigma^2 \\sim IG(a,b)\$. As we are not experts in the field of predicting home prices, we do not know the specifics of the variability of home prices in Boston. Therefore, we chose to set the prior for^ $\sigma^2$ to a non-informative prior. We decided to choose the $IG(1,1)$ prior for $\sigma^2$ as we believe that $\sigma^2$ is mostly likely small due to the high quality source of the data, but there might be ....
