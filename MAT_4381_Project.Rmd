...
Now that we have the likelihood function and the prior probability distribution of the parameters, we can now compute the posterior distribution of the parameters. We will follow a similar procedure to [3]. We let $h(\boldsymbol{\beta}, \sigma^2 | \boldsymbol{y}, X)$ represent the posterior distribution function:
...
We now looked at model diagnostics. We find that we can assume that the variance of the error terms are constant and we can also assume that the relationship is linear from looking at the residual vs. fitted plot. This is because the points don't follow a noticeable pattern. However, from looking at the qq-plot, the assumption that the random errors are normally distributed may not be reasonable for this specific dataset.
...
```{=latex}
Figure 1: Histogram of Simulated Distribution of Prior for $\sigma^2$ ($IG(5, 75)$)
\newpage
```
...
```{=latex}
Figure 2: Histograms of Simulated Marginal Posterior Distributions for Beta Coefficients of Predictors (with labels for 95\% credible regions and mean of posterior distributions)
```
...
```{=latex}
Figure 3: Histogram of Simulated Posterior Distribution for $\sigma^2$
\newpage
```
...
beta_estimates_comparison['Beta Estimate Magnitude Difference (|OLS| - |Bayesian|)'] = abs(beta_estimates['Best Posterior Beta Estimate (under Squared Error Loss)']) - abs(OLS_beta_estimates['betaEstimate'])
...
\\
Table 2: Comparison of the ordinary least squares estimates and the Bayes estimates of the parameters under the squared error loss of the model parameters.
```
...
5.1 - Ridge Regression. PennState. (2018). https://online.stat.psu.edu/stat857/node/155/ 
