...
We have assumed that $\boldsymbol{\epsilon} \sim N(\boldsymbol{0}, \sigma^2 I)$. This consequently leads to that $\boldsymbol{y}|X \sim N(X\boldsymbol{\beta}, \sigma^2I)$. Under this assumption, we can also get that $$\hat{\boldsymbol{\beta}} \sim N(\boldsymbol{\beta}, \sigma^2(X^TX)^{-1})$$
...
Note that the above explanation of OLS regression was based on Montgomery, Peck, & Vining, 2021, pp. 12-91.
...
Now we have established the theory behind Bayesian linear regression, we will compare the Bayesian linear regression model to the ordinary least squares model on the Boston Housing dataset. The data was derived from information collected by the U.S. Census Service concerning housing in the area of Boston, Massachusetts (Harrison & Rubinfeld, 1978). Note that it is believed that the median house prices were censored past 50 (\$50,000).
...
We have that the full frequentist linear regression model fitted using the training dataset is $\hat{Y} = 32.0445 - 0.122 X_1 + 0.043 X_2 + 0.067 X_3 - 13.995 X_4 + 4.022 X_5 - 0.005 X_6 - 1.470 X_7 + 0.343 X_8 - 0.014 X_9 - 0.856 X_{10} + 0.012 X_{11} - 0.063 X_{12}$, where the variables are as described in the previous section. From looking at the summary table of the model, we see that the variables $X_3$ (INDUS) and $X_6$ (AGE) are not statistically significant under the significance level $\alpha = 0.05$ because the p-values from the t tests are greater than 0.05.\
...
Now, we build a reduced linear regression model using stepwise selection (allow predictors to enter the model with p-value of t-test less than 10% and be removed with p-value of t-test greater than 15% (p-value/t-test is computed based on the model developed at that current step of the stepwise process)):
...
After evaluating the model, we obtain a training MSE of 21.730 and test MSE of 24.527. The training MSE that accounts for the number of parameters in the model is around 22.540, which is slightly lower than the one for the full model (which was 22.627). This indicates that the reduced model, when penalizing for the increase in parameters in the full model, explains the model slightly better than the full model (captures less noise). Note that the test MSE is also slightly lower with this model, which is a further confirmation that removing the non-significant predictors $X_3$ and $X_6$ from the model improved predictions ever so slightly (on another test dataset the difference between the two test MSE may be larger). Remember that the median house prices are given in \$1000s, so the test MSE signify that the mean square error on the test dataset for the full and reduced models are around \$24,705 and \$24,527, respectively.
...
We now execute the algorithm for finding the posterior distribution for $\sigma^2$ and the marginal posterior distributions for $\beta_i, i \in \{1, \dots, 12\}$, with the $i^{th}$ $\beta$ coefficeint ($\beta_i$) corresponding to the $i^{th}$ predictor ($X_i$) ($\beta_0$ is intercept term).
...
for i in range(num_predictors):
    ...
    low_cr = subaxis.axvline(x=value_at_25, color='r', 
                             linestyle='--', linewidth=5, label = 'Lower 95% CR Bound for $\\beta_i$ Posterior')
    high_cr = subaxis.axvline(x=value_at_975, color='darkred', 
                              linestyle='--',linewidth=5, label = 'Upper 95% CR Bound for $\\beta_i$ Posterior')
    mean_est = subaxis.axvline(x=mu_post[i], color='c', 
                               linestyle='--', linewidth=5, 
                               label = '$\\beta_i$ A Posterior Best Estimate (under Squared Error Loss)')
...
```{=latex}
Figure 3: Histograms of Simulated Marginal Posterior Distributions for Beta Coefficients of Predictors (with labels for 95\% credible regions and mean of posterior distributions (best estimate for each beta coefficient under squared error loss))
```
```{python}
