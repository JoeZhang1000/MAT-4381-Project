...
#Ordinary Least Squares Regression Revisited (based on Montgomery, Peck, & Vining, 2021, pp. 12-91)
...
Now we have established the theory behind Bayesian linear regression, we will compare the Bayesian linear regression model to the ordinary least squares model on the Boston Housing dataset (Harrison & Rubinfeld, 1978).
...
\end{itemize}
```
Dataset Source: The data were derived from information collected by the U.S. Census Service concerning housing in the area of Boston, Massachusetts. Note that it is believed that the median house prices were censored past 50 ($50,000).
...
ggplot(train_set_dataset, aes(x = MEDV)) +
  ...
  labs(title = "Histogram of Median value of owner-occupied homes in $1000s (MEDV)", 
       x = "Median Value of Owner-Occupied Homes ($1000s)", y = "Frequency") + 
  theme_minimal() 
...
As seen in Figure 1, the response variable MEDV is somewhat normally distributed, with some outliers around 50. This is a result of the median house prices being censored past $50,000 in the dataset. 


## Ordinary Least Squares Model

We first fit a linear regression model under the frequentist framework. Then we make inference and evaluate the quality of the model. We consider 2 possible models: one using all predictors and one that is selected using stepwise selection. (Note we will not talk about $R^2$ or ANOVA/F-tests between full and reduced models as these are not directly applicable to our Bayesian setting).

We first fit the full model with all variables:

We have that the full frequentist linear regression model fitted using the training dataset is $\hat{Y} = 32.044484 - 0.121517 X_1 + 0.042545 X_2 + 0.066926 X_3 - 13.994681 X_4 + 4.021977 X_5 - 0.004531 X_6 - 1.469667 X_7 + 0.343192 X_8 - 0.014404 X_9 - 0.855679 X_{10} + 0.011628 X_{11} - 0.0629198 X_{12}$, where the variables are as described in the previous section. From looking at the summary table of the model, we see that the variables $X_3$ (INDUS) and $X_6$ (AGE) are not statistically significant under the significance level $\alpha = 0.05$ because the p-values from the t tests are greater than 0.05.
...
cat("Training MSE (adjusted for #parameters):", 
    training_MSE*nrow(train_set_dataset)/(nrow(train_set_dataset) - length(coef(full_model))), "\n")
...
We then evaluated the model using both the training dataset and the test dataset. The training error is 21.66596 and the test error is 24.70547. Note that the MSE used does not account for the number of parameters in the model as we wanted to be consistent between the training and test MSE computations, where in the test setting the number of parameters is not as relevant (want lowest test error independent of #parameters). If we accounted for the number of parameters in the model, we would get a training MSE of 22.62725.

Now, we build a reduced linear regression model using stepwise selection (allow predictors to enter the model with p-value of t-test less than 10% and be removed with p-value of t-test greater than 15% (p-value/t-test is computed based on the model at that current step of the stepwise process)):
...
cat("Training MSE (adjusted for #parameters):", 
    training_MSE*nrow(train_set_dataset)/
...
We have that the reduced model selected via stepwise regression is $\hat{Y} = 31.664864 - 0.123388X_1 + 0.042781X_2 - 13.171578X_4 + 3.951521X_5 -1.507618X_7 + 0.325008 X_8 - 0.012985 X_9 - 0.823535 X_{10} + 0.011428 X_{11} - 0.626362X_{12}$. All predictors are statistically significant in this model based on the t-tests, indicating that we removed the non-significant predictors from the full model that were not significantly explaining the median home prices. 

After evaluating the model, we obtain that the training MSE is 21.73032 and the test MSE is 24.52705. The training MSE that accounts for the number of parameters in the model is 22.5406, which is slightly lower than the one for the full model. This indicates that the reduced model, when penalizing for the increase in parameters in the full model, explains the model slightly worse than the reduced model. Note that the test MSE is also slightly lower with this model. 
...
```{=latex}
Figure 2: Histogram of Simulated Distribution of Prior for $\sigma^2$ ($IG(5, 75)$)
\newpage
```

**Algorithm**

We now execute the algorithm for finding the posterior distribution for $\sigma^2$ and the marginal posterior distributions for $\beta_i, i \in \{1, \dots, 12\}$, with the $i^{th}$ $\beta$ corresponding to the $i^{th}$ predictor ($X_i$) ($\beta_0$ is intercept term).
...
#Compute alpha and r paramater of sigma2 posterior distribution (inverse gamma)
...
beta_estimates = pd.DataFrame(index=train_set.columns[0:num_predictors], 
                              columns=["Best Posterior Beta Estimate (under Squared Error Loss)", "95% Credible Region"])
...
for i in range(num_predictors):
    ...
    low_cr = subaxis.axvline(x=value_at_25, color='r', 
                             linestyle='--', linewidth=5, label = 'Lower 95% CR Bound for Estimate')
    high_cr = subaxis.axvline(x=value_at_975, color='darkred', 
                              linestyle='--',linewidth=5, label = 'Upper 95% CR Bound for Estimate')
    mean_est = subaxis.axvline(x=mu_post[i], color='c', 
                               linestyle='--', linewidth=5, 
                               label = 'Posterior Beta Coefficient Estimate for Predictor')
...
```{=latex}
Figure 3: Histograms of Simulated Marginal Posterior Distributions for Beta Coefficients of Predictors (with labels for 95\% credible regions and mean of posterior distributions)
```
...
\\
\\
Table 1: Summary of the Bayes estimates for the model parameters. It contains the Bayes estimates under the squared error loss (mean of posterior) and the 95\% credible region.
```
We can see in Table 1 that our estimated Bayesian linear regression model is $\hat{Y} = 29.2027 - 0.119609X_1 + 0.0434526X_2 + 0.0626662X_3 - 12.4738 X_4 + 4.16421 X_5 - 0.00535482 X_6 - 1.43013 X_7 + 0.333189 X_8 - 0.0142721 X_9 - 0.807487 X_{10} + 0.0119776 X_{11} - 0.624253 X_{12}$. From the 95% credible regions of the parameters, we can see that for the predictors $X_3$ (INDUS) and $X_6$ (AGE), their credible regions contain 0. In the hypothesis testing framework, we are testing the null hypothesis that the beta coefficient for the predictor equals 0 agains the alternative that it does not equal 0. 0 being contained in the 95% credible regions for the Beta coefficients of $X_3, X_6$ indicates that these predictors are not statistically significant to explaining the median house prices in Boston towns (we accept the null hypothesis that the beta coefficients equal 0). The credible regions for the other parameters don't contain 0, which means that they are statistically significant to the model (we reject the null hypothesis that the beta coefficients equal 0).
...
We can see that the training MSE of our Bayesian model on a training set is around 21.68 (not adjusted for #parameters), while the test MSE is higher at around 24.87. As the training and test MSE are similar, there does not seem to be too much overfitting occurring.
...
beta_estimates_comparison['Beta Estimate Difference (|OLS| - |Bayesian|)'] = abs(beta_estimates['Best Posterior Beta Estimate (under Squared Error Loss)']) - abs(OLS_beta_estimates['betaEstimate'])
beta_estimates_comparison.insert(0, 'Full OLS Beta Estimate', OLS_beta_estimates['betaEstimate'])
...
```{python, echo=FALSE, warning=FALSE, message=FALSE}
OLS_beta_estimates = pd.read_csv('stepwise_model_summary.csv', index_col=0).rename(index={'(Intercept)': 'Intercept'})
...
beta_estimates_comparison['Beta Estimate Difference (|OLS| - |Bayesian|)'] = abs(beta_estimates['Best Posterior Beta Estimate (under Squared Error Loss)'].drop(["AGE", "INDUS"], axis=0)) - abs(OLS_beta_estimates['betaEstimate'])
beta_estimates_comparison.insert(0, 'Stepwise OLS Beta Estimate', OLS_beta_estimates['betaEstimate'])
...
\\
Table 2: Comparison of the ordinary least squares estimates (full model) and the Bayes estimates of the parameters under the squared error loss of the model parameters.
```
As seen in Table 2, we can see that the Bayesian estimates for the $\beta$ coefficients are not much different compared to the ordinary least squares estimates. The last column of Table 2 shows the difference between the Bayesian and ordinary least squares estimates. The magnitude of the differences are all not very high (less than 1). This shows that the estimated linear regression models under both the ordinary least squares and the Bayesian framework are very similar. This is due to choosing a small $\lambda$ value in the Bayesian framework (the smaller $\lambda$ is chosen the less penalty is applied to large beta coefficients), resulting in a minor shrinkage in the beta coefficient estimates in the Bayesian setting. This small shrinkage in the Bayesian Beta coefficients estimates can be seen in the last column of the table. Negative values indicate that the Bayesian best estimates shrunk towards 0 compared to the ordinary least squares estimates, while positive values indicates they grew away from 0. It is evident that for the majority of Beta coefficients, our best estimates under the Bayesian setting became smaller than the ordinarly least squares best estimates.

```{=latex}
\input{reduced_beta_estimates.tex}
\\
Table 3: Comparison of the ordinary least squares estimates (reduced/stepwise model) and the Bayes estimates of the parameters under the squared error loss of the model parameters.
```

Similarly to the full model, the reduced/stepwise model produces similar results to the Bayesian model for the Beta coefficient estimates as seen in Table 3. Among most coefficient estimates, the differences are slightly smaller than with the full model. We also have more beta coefficients in the Bayesian model that are slightly greater than the reduced model estimates than with the full model.

If we look at the training and test MSE from all three models, they are all extremely similar, indicating that they all fit the model relatively the same. 

```{=latex}
\input{beta_95_CI_CR_table.tex}
\\

Table 4: Comparison of the 95\% confidence intervals (from full OLS model) and the 95\% credible regions of the $\beta$ parameters.
```
...
training_MSE_adj <- (training_MSE*nrow(train_set_dataset))/
...
We now compare the variance of the response variable (Median House Prices) under the ordinary least squares and the Bayesian framework. Under the ordinary least squares framework, the estimate for the variance is the mean squared error (we use the full model to be consistent among methods), or the mean sum of squares residuals. We obtain an estimate of 22.694 (the training MSE from OLS). Under the Bayesian framework, we get that the estimate for the variance under the squared error loss is 21.736 and the MAP (Maximum a Posterior) estimate is 21.462. This shows that the estimate for the variance of the error terms of the median house prices is similar under both the ordinary least squares and the Bayesian framework.

# Conclusion/Future Steps

All in all, we explored the differences between Ordinary Least Squares Regression (Frequentist Method) and the Bayesian method of settings priors to find posterior distributions for the model parameters. We analyzed the Boston Housing dataset using both OLS and Bayesian models. Overall, we found that the OLS and Bayesian estimates for the Beta coefficients and $\sigma^2$ were similar in both frameworks. We note that utilizing different parameters for the priors in the Bayesian model could lead to vastly different estimates and affect the significance of the predictors. In the future, we could further explore in the Bayesian model how to create predictivie posterior distributions to predict new observations, hypothesis testing of multiple parameters, creating joint credible regions, choosing different types of distributions as priors (such as Jeffrey's prior), and transformations of our data.
